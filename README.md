...
> [!IMPORTANT]
> An AI-generated fact, attribution, URL, date, translation, or confidence
> label is a review lead, not proof. Only approve an item after opening and
...
Each record has a collection, attribution, source metadata, approval state,
verification status, and internal note. Latin records additionally use Latin
wording, translation, and Latin classification. Fact records use a lead,
...
3. Review bundled, imported, translated, or AI-generated content.
4. Independently verify the wording, source, URL, attribution, and metadata.
...
records, and refill the queue only after saving your collection settings.
...
The app calls the worker start check on every Streamlit rerun. The check is a
no-op while the cached worker thread is alive and revives it if that thread has
stopped. New automation cannot be enabled unless the worker reports `Active`.
...
Critical approval, schedule, automation, and cancellation writes are read back
from GitHub before the app reports success. Wait for the persistent
...
If verification fails, refresh once and inspect the displayed saved count
before trying again.

### The worker is Starting, Stale, Stopped, or Error

1. Pause automation.
2. Select **Refresh dashboard** once.
3. Wait approximately one worker interval and refresh again.
4. If the worker does not become **Active**, open **Manage app** and reboot the
   Streamlit app.
5. Confirm the queue and LinkedIn connection remain present.
6. Re-enable automation only after the worker is Active.
